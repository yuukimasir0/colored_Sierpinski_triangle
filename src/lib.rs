...
pub fn main_js() -> Result<(), JsValue> {
    ...
    sierpinski(&context, [(300.0, 0.0), (0.0, 600.0), (600.0, 600.0)], (200, 12, 80), 5);
    Ok(())
}
...
fn sierpinski(context: &web_sys::CanvasRenderingContext2d, points: [(f64, f64); 3], color: (u8, u8, u8), depth: u8) {
    ...
    if depth > 0  {
        ...
        let bottom = midpoint(left, right);
        sierpinski(&context, [top, left_mid, right_mid], next_color, depth);
        sierpinski(&context, [left_mid, left, bottom], next_color, depth);
        sierpinski(&context, [right_mid, bottom, right], next_color, depth);
    }
}
